...
$(function(){
	...
	$("#scname").blur(function(){
		inusername=$(this).val();
		if(inusername != null){
			$.post("${pageContext.request.contextPath }/verifySCname",{"name":inusername},
					function(data){
						//获取到返回来的植
						//var reusername = "";
						if(data != null){
							reusername=data.scname;
						}
...
<div class="page-content">
 ...
 <div class="user_Manager_style">
    ...
    <div id="win" class="easyui-window" style="height: 250px;width: 310px" data-options="title:'数据写入',closable:false,draggable:false,iconCls:'icon-add',resizable:false,minimizable:false,maximizable:false,closed:true,resizable:false,modal:true,shadow:true">
    	<form id="addForm" action="addSpotCheck">
    		<label class="label_name">名称:</label>
    		<input type="text" name="scname" id="scname" class="easyui-validatebox" required="true"><span id="nameinfo"></span>
    	</form>
    		<br><br><br><br><br><br>
    ...
    <div id="tb">
     ...
     $("#tab").datagrid({
 		url:'${pageContext.request.contextPath}/show_spotcheck',
			...
			columns:[
						[
							...
							{align:'center',width:300, title:'任务名',			field:'scname'}
						 ]
					], 
			//显示序号rownumbers
			rownumbers:true,
			...
			toolbar:[
    			...
    			{text:'<input id="sear" value="" class="easyui-searchbox" style="width:152px;height:19px;margin-top: -15px">'},
    		],
			toolbar:'#tb',
     ...
     function deleter(){
    	...
    	var items = $("#tab").datagrid('getSelections');
    	 ...
    	 $.ajax({
    		 ...
    		 traditional: true,
    		 async:false,
    		 url:'${pageContext.request.contextPath}/deleterSpotCheck',
    		 data:{"ids":ids},
    		 success:function(data){
    			 if(data.data==300){
