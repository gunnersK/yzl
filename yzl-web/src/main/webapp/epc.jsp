...
$(function(){
	...
	$("#ename").blur(function(){
		inusername=$(this).val();
		if(inusername != null){
			$.post("${pageContext.request.contextPath }/verifyName",{"name":inusername},
					function(data){
						//获取到返回来的植
						//var reusername = "";
						if(data != null){
							reusername=data.ename;
						}
		 ...
		 if(reusername!=inusername){
			 ...
			 $("#addForm").form('submit',{
				success:function(data){
					//alert(data.data);
					if(data != null){
						$.messager.alert("提示框","插入成功 ! !",'info');
					}else if(data == null){
						$.messager.alert("提示框","插入失败 ! !",'info');
					}
...
<div class="page-content">
 ...
 <div class="user_Manager_style">
    ...
    <!-- 窗口 -->
    <div id="win" class="easyui-window" style="height: 250px;width: 310px" data-options="title:'数据写入',draggable:false,iconCls:'icon-add',resizable:false,minimizable:false,maximizable:false,closed:true,resizable:false,modal:true,shadow:true">
    	<form id="addForm" action="addEpc">
    		<label class="label_name">工程名称:</label>
    		<input type="text" name="ename" id="ename" class="easyui-validatebox" required="true"><span id="nameinfo"></span>
    	</form>
    		<br><br><br><br><br><br>
    ...
    <div id="tb">
     ...
     $("#tab").datagrid({
 		url:'${pageContext.request.contextPath}/show_epc',
			...
			columns:[
						[
							{align:'center',width:100, title:'序列化',checkbox:true, 	field:'ecode'},
							{align:'center',width:300, title:'工程名',			field:'ename'},
						 ]
					], 
			//显示序号rownumbers
			rownumbers:true,
    	...
    	function add(){
     	...
     	function deleter(){
     		...
     		if(items.length == 0){
     			$.messager.alert("提示框","必须选者一个",'info');
     			return;
     		}
     		
     		var ids=[];
     		for(var i=0;i<items.length;i++){
     			ids.push(items[i].ecode);
     		}
     		$.messager.confirm('警告！','确定要删除当前工程吗？',function(r){
     		 if(r){
	     		$.ajax({
	     			url:'${pageContext.request.contextPath }/deleteEpc',
	     			type:'post',
	     			async:false,
	     			traditional: true,
	     			data:{"ids":ids},
	     			success:function(data){
	     				if(data.data==300){
	     					$.messager.alert("提示框","此工程与其他任务有关联,不可删除哦 ! !","info");
	     				}
	     				if(data.data==200){
