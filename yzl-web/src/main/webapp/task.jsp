...
$(function(){
	...
	$("#tname").blur(function(){
		inusername=$(this).val();
		if(inusername != null){
			$.post("${pageContext.request.contextPath }/verifyTname",{"name":inusername},
					function(data){
						//获取到返回来的植
						//var reusername = "";
						if(data != null){
							reusername=data.tname;
						}
...
<div class="page-content">
 ...
 <div class="user_Manager_style">
    ...
    <div id="win" class="easyui-window" style="height: 250px;width: 310px" data-options="title:'数据写入',draggable:false,iconCls:'icon-add',resizable:false,minimizable:false,maximizable:false,closed:true,resizable:false,modal:true,shadow:true">
    	<form id="addForm" action="addTask">
    		<label class="label_name">任务名称:</label>
    		<input type="text" name="tname" id="tname" class="easyui-validatebox" required="true"><span id="nameinfo"></span>
    	</form>
    		<br><br><br><br><br><br>
    ...
    <div id="tb">
     ...
     $("#tab").datagrid({
 		url:'${pageContext.request.contextPath}/show_task',
			...
			columns:[
						[
							{align:'center',width:100, title:'序列号',checkbox:true, 	field:'tcode'},
							{align:'center',width:300, title:'任务名',			field:'tname'}
						 ]
					], 
			//显示序号rownumbers
			rownumbers:true,
     ...
     function deleter(){
    	...
    	var items = $("#tab").datagrid('getSelections');
    	 ...
    	 for(var i=0;i<items.length;i++){
    		 ids.push(items[i].tcode);
    	 }
    	 $.ajax({
    		 type:'post',
    		 traditional: true,
    		 async:false,
    		 url:'${pageContext.request.contextPath}/deleterTask',
    		 data:{"ids":ids},
    		 success:function(data){
    			 if(data.data==300){
